...
func prWithReviewState(s model.AggregateReviewState) model.PullRequest {
	return model.PullRequest{Reviews: model.ReviewSummary{AggregateState: s}}
}

func TestGroupBy_None(t *testing.T) {
	prs := []model.PullRequest{prWithRepo("a", "x"), prWithRepo("b", "y")}

	groups := query.GroupBy(prs, query.GroupSpec{By: query.GroupNone})
...
func TestGroupBy_Repo(t *testing.T) {
	...
	groups := query.GroupBy(prs, query.GroupSpec{By: query.GroupRepo})
...
func TestGroupBy_Provider(t *testing.T) {
	...
	groups := query.GroupBy(prs, query.GroupSpec{By: query.GroupProvider})
...
func TestGroupBy_Author_SortedByCount(t *testing.T) {
	...
	groups := query.GroupBy(prs, query.GroupSpec{By: query.GroupAuthor})
...
func TestGroupBy_ReviewStatus_TriagePriority(t *testing.T) {
	...
	groups := query.GroupBy(prs, query.GroupSpec{By: query.GroupReviewStatus})
	...
	for i, want := range wantOrder {
		if groups[i].Key != want {
			t.Errorf("position %d: got %q, want %q", i, groups[i].Key, want)
		}
	}
}

func TestGroupBy_ValidateForResource(t *testing.T) {
	spec := query.GroupSpec{By: query.GroupReviewStatus}

	if err := spec.ValidateForResource("pr"); err != nil {
		t.Errorf("expected GroupReviewStatus to be valid for pr, got error: %v", err)
	}
	if err := spec.ValidateForResource("issue"); err == nil {
		t.Error("expected GroupReviewStatus to be invalid for issue resource type")
	}
}

func TestGroupBy_ZeroSpec(t *testing.T) {
	prs := []model.PullRequest{prWithRepo("a", "x")}

	groups := query.GroupBy(prs, query.GroupSpec{})
