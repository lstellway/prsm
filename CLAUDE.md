...
| `prsm` (root) | Partial | Assembly layer — `New`/`NewWithAdapters` construct connections from config and index them by resource-kind interface via type assertion. GitHub only; `gitlab`/`gitea` provider types produce a construction error until those adapters exist. Identity resolution, snapshot fetch, and the poll loop are not yet built |
